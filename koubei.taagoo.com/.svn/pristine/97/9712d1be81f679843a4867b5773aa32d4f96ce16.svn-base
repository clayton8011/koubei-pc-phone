...
class BaseController extends Controller
{
    public function beforeAction($action)
    {
        if (Yii::$app->user->isGuest) {
            $login_url = Yii::$app->params['aliyun_app']['openauth'] . '?app_id=' . Yii::$app->params['aliyun_app']['appId'] . '&redirect_uri=' . urlencode(Yii::$app->params['aliyun_app']['redirect_uri'] . '?id=shop&return_url=' . urlencode(urlencode(Yii::$app->request->hostInfo . Yii::$app->request->getUrl())));
            $user_id = Yii::$app->session->get('pano_shop_session');
            if (!$user_id) {
